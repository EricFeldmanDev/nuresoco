...
class ShoppingCart
{
    ...
    public function manageShoppingCart()
    {
        ...
        if ($_POST['action'] == 'add') {
			...
			$totalInCart = count($result['array']);
    ...
    public function getCartItems()
    {
        ...
        foreach ($result['array'] as $key => $article) {
			...
			if($_SESSION['shopping_cart'])
			{
				...
				$article['sum_price'] = ($article['price'] * $cart_quantity) + ($article['shipping'] * $cart_quantity);
				...
				$article['price'] = number_format((int)$article['price'], 2);
		...
		$result['totalInCart']=count($result['array']);
