<div id="products">
    ...
    ?>
    <h1><img src="<?= base_url('assets/imgs/products-img.png') ?>" class="header-img" style="margin-top:-2px;"> Accepted Products</h1>
    <hr>
    <div class="row">
        <div class="col-xs-12">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            foreach ($products as $row) { //pr($row,1);
                                ...
                                //echo base_url('assets/uploads/product-images/'.$row->image); 
                                if ($row->image != null) { 
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $html = 'Add'; $label = array();
                                        if($row->filter_label){
                                            $label = json_decode($row->filter_label, true);                                            
                                            $style = '';
                                            if($label['bold']){
                                        ...
                                        echo '<a href="javascript:" class="edit-label" data-id="'.$row->product_id.'" data-content=\''.json_encode($label).'\'>'.$html.'</a>';
                                        ?>
                                    </td>
                                    <td>
										<?=$row->is_product_verified=='1'?'Verified':'Not Verified' ?>
                                    </td>
                                    <td>
                                        <div>
											<a href="javascript:void(0)<?php //echo base_url('admin/publish/' . $row->product_id) ?>" class="btn btn-info">View</a>
											<?php if($row->is_product_verified=='1'){ ?>
												<a href="<?php echo base_url('admin/unverifyProduct/' . base64_encode($row->product_id)) ?>" class="btn btn-primary">UnVerify</a>
											<?php }else{ ?>
												<a href="<?php echo base_url('admin/verifyProduct/' . base64_encode($row->product_id)) ?>" class="btn btn-primary">Verify</a>
											<?php } ?>
                                            <a href="<?= base_url('admin/acceptedProducts?delete=' . base64_encode($row->product_id)) ?>"  class="btn btn-danger confirm-delete">Delete</a>
                                            <a   class="btn btn-primary" href="<?php echo base_url('admin/review/' . base64_encode($row->product_id)) ?>"> Review</a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Product Filter Label</h4>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary btn-save-label">Save</button>
